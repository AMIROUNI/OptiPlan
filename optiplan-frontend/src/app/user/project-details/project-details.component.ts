...
export class ProjectDetailsComponent implements OnInit {
  ...
  getInReviewTasksCount(): number {
    return this.tasks.filter(t => t.status === 'InReview').length;
  }
  ...
  getStatusClass(status: string): string {
    switch (status) {
      ...
      case 'InReview': return 'status-in-review';
  ...
  filterTasks(status: 'all' | 'todo' | 'inprogress' | 'done' | 'review' = 'all'): void {
    ...
    switch (status) {
      case 'todo':
        ...
        break;
      case 'inprogress':
        this.filteredTasks = this.tasks.filter(task => task.status === 'InProgress');
        break;
      case 'done':
        this.filteredTasks = this.tasks.filter(task => task.status === 'Done');
        break;
      case 'review':
        this.filteredTasks = this.tasks.filter(task => task.status === 'InReview');
        break;
      default:
